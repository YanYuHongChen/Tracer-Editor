...
void Direct3D11Renderer::reset(const uint flags)
{
  ...
  if (flags & RESET_TEX)
  {
    for (uint i = 0; i < MAX_TEXTUREUNIT; i++)
    {
      ...
      selectedTexturesUAVPS[i] = NO_UAV;
...
TextureID Direct3D11Renderer::addTexture(Image &img, const SamplerStateID samplerState, uint flags)
{
  ...
  if (FAILED(hr))
  {
    g_logger->debug(WIP_ERROR,"Couldn't create texture");
    return TEXTURE_NONE;
  }
...
TextureID Direct3D11Renderer::addRenderTarget(const int width, const int height, const int depth, const int mipMapCount, const int arraySize, const FORMAT format, const int msaaSamples, const SamplerStateID samplerState, uint flags)
{
  ...
  if (depth == 1)
  {
    ...
    {
      g_logger->debug(WIP_ERROR,"Couldn't create render target");
      return TEXTURE_NONE;
    }
  }
  else
...
TextureID Direct3D11Renderer::addRenderDepth(const int width, const int height, const int arraySize, const FORMAT format, const int msaaSamples, const SamplerStateID samplerState, uint flags)
{
  ...
  if (FAILED(device->CreateTexture2D(&desc, NULL, (ID3D11Texture2D **)&tex.texture)))
  {
    g_logger->debug(WIP_ERROR, "Couldn't create depth target");
    return TEXTURE_NONE;
  }
  ...
  const char *header, const char *extra, const char *fileName, const char **attributeNames, const int nAttributes, const uint flags)
{
  ...
  UINT compileFlags = D3DCOMPILE_PACK_MATRIX_ROW_MAJOR | D3DCOMPILE_ENABLE_STRICTNESS;
    //| D3DCOMPILE_ENABLE_BACKWARDS_COMPATIBILITY | D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION;// | D3D11_SHADER_DEBUG | D3D11_SHADER_SKIP_OPTIMIZATION;
  ...
  if (vsText != NULL)
  {
    ...
    if (SUCCEEDED(D3DCompile(shaderString.data(), shaderString.length(), fileName, NULL, NULL, "main", target, compileFlags, 0, &shaderBuf, &errorsBuf)))
    {
      if (SUCCEEDED(device->CreateVertexShader(shaderBuf->GetBufferPointer(), shaderBuf->GetBufferSize(), NULL, &shader.vertexShader)))
      {
        ...
        HRESULT ret = D3DReflect(shaderBuf->GetBufferPointer(), shaderBuf->GetBufferSize(), IID_ID3D11ShaderReflection, (void **)&vsRefl);
        if (ret != S_OK)
        {
          g_logger->debug(WIP_ERROR, "Reflect VS failed! Code:%x ",ret);
        }
        ...
        if (flags & ASSEMBLY)
        {
          ...
          {
            g_logger->debug(WIP_ERROR,(const char *)disasm->GetBufferPointer());
          }
          SAFE_RELEASE(disasm);
        }
#endif
    ...
    {
      g_logger->debug(WIP_ERROR,(const char *)errorsBuf->GetBufferPointer());
    }
  ...
  if (fsText != NULL)
  {
    ...
    if (SUCCEEDED(D3DCompile(shaderString.data(), shaderString.length(), fileName, NULL, NULL, "main", target, compileFlags, 0, &shaderBuf, &errorsBuf)))
    {
      ...
      {
        HRESULT hr = D3DReflect(shaderBuf->GetBufferPointer(), shaderBuf->GetBufferSize(), IID_ID3D11ShaderReflection, (void **)&psRefl);
        if (hr != S_OK)
        {
          g_logger->debug(WIP_ERROR, "Reflect PS failed! Code:%x ",hr);
        }
        ...
        if (flags & ASSEMBLY)
        {
          ...
          if (SUCCEEDED(D3DDisassemble(shaderBuf->GetBufferPointer(), shaderBuf->GetBufferSize(), 0, NULL, &disasm)))
          {
            g_logger->debug(WIP_INFO,(const char *)disasm->GetBufferPointer());
          }
          SAFE_RELEASE(disasm);
        }
  ...
  if (vsRefl)
  {
    ...
    if (vsDesc.ConstantBuffers)
    {
      ...
      shader.vsConstMem = new u8 *[shader.nVSCBuffers];
  ...
  if (gsRefl)
  {
    ...
    if (gsDesc.ConstantBuffers)
    {
      ...
      shader.gsConstMem = new u8 *[shader.nGSCBuffers];
  ...
  if (psRefl)
  {
    ...
    if (psDesc.ConstantBuffers)
    {
      ...
      shader.psConstMem = new u8 *[shader.nPSCBuffers];
  ...
  for (uint i = 0; i < shader.nVSCBuffers; i++)
  {
    ...
    shader.vsConstMem[i] = new u8[sbDesc.Size];
  ...
  for (uint i = 0; i < shader.nGSCBuffers; i++)
  {
    ...
    shader.gsConstMem[i] = new u8[sbDesc.Size];
    for (uint k = 0; k < sbDesc.Variables; k++)
    {
      D3D11_SHADER_VARIABLE_DESC vDesc;
  ...
  for (uint i = 0; i < shader.nPSCBuffers; i++)
  {
    ...
    shader.psConstMem[i] = new u8[sbDesc.Size];
    for (uint k = 0; k < sbDesc.Variables; k++)
    {
      D3D11_SHADER_VARIABLE_DESC vDesc;
  ...
  if (maxResources)
  {
    ...
    for (uint i = 0; i < nMaxVSRes; i++)
    {
	  ...
	  if (siDesc.Type == D3D10_SIT_TEXTURE || siDesc.Type == D3D_SIT_STRUCTURED)
      {
...
ShaderID Direct3D11Renderer::addShader(ID3DBlob *vs, ID3DBlob *gs, ID3DBlob *ps, pD3DReflect reflect_shader)
{
	if (vs == NULL && gs == NULL && ps == NULL)
		return SHADER_NONE;
	...
	if (vs != NULL)
	{
		...
		if (SUCCEEDED(device->CreateVertexShader(vs->GetBufferPointer(), vs->GetBufferSize(), NULL, &shader.vertexShader)))
		{
			HRESULT res;
			res = D3DGetInputSignatureBlob(vs->GetBufferPointer(), vs->GetBufferSize(), &shader.inputSignature);
			if (res != S_OK)
				g_logger->debug(WIP_ERROR, "D3DGetInputSignatureBlob vs faled!!%x", res);
			if (reflect_shader)
			{
				res = reflect_shader(vs->GetBufferPointer(), vs->GetBufferSize(), IID_ID3D11ShaderReflection, (void **)&vsRefl);
				if (res != S_OK)
					g_logger->debug(WIP_ERROR, "reflect vs faled!!%x", res);
			}
		}
	...
	if (gs != NULL)
	{
		if (SUCCEEDED(device->CreateGeometryShader(gs->GetBufferPointer(), gs->GetBufferSize(), NULL, &shader.geometryShader)))
		{
			if (reflect_shader)
			{
				HRESULT res = reflect_shader(gs->GetBufferPointer(), gs->GetBufferSize(), IID_ID3D11ShaderReflection, (void **)&gsRefl);
				if (res != S_OK)
					g_logger->debug(WIP_ERROR, "reflect gs faled!!%x", res);
			}
		}
		if (shader.geometryShader == NULL)
			return SHADER_NONE;
	...
	if (ps != NULL)
	{
		if (SUCCEEDED(device->CreatePixelShader(ps->GetBufferPointer(), ps->GetBufferSize(), NULL, &shader.pixelShader)))
		{
			if (reflect_shader)
			{
				HRESULT res = reflect_shader(ps->GetBufferPointer(), ps->GetBufferSize(), IID_ID3D11ShaderReflection, (void **)&psRefl);
				if (res != S_OK)
					g_logger->debug(WIP_ERROR, "reflect ps faled!!%x", res);
			}
		}
		if (shader.pixelShader == NULL)
			return SHADER_NONE;
	...
	uint nMaxGSRes = gsRefl ? gsDesc.BoundResources : 0;
	uint nMaxPSRes = psRefl ? psDesc.BoundResources : 0;
	//printf("samplers: %d,%d,%d\n", vsDesc.BoundResources,gsDesc.BoundResources,psDesc.BoundResources);
	int maxResources = nMaxVSRes + nMaxGSRes + nMaxPSRes;
	if (maxResources)
	{
		shader.textures = (Sampler *)malloc(maxResources * sizeof(Sampler));
		shader.samplers = (Sampler *)malloc(maxResources * sizeof(Sampler));
		shader.uavs = (Sampler *)malloc(maxResources * sizeof(Sampler));
		...
		{
			psRefl->GetResourceBindingDesc(i, &siDesc);
			if (siDesc.Type == D3D10_SIT_TEXTURE || siDesc.Type == D3D_SIT_STRUCTURED || siDesc.Type == D3D_SIT_UAV_RWTYPED)
			{
				int merge = -1;
				for (uint i = 0; i < maxTexture; i++)
				{
			...
			else if (siDesc.Type == 12)
			{
				//never used
				int merge = -1;
				for (uint i = 0; i < maxTexture; i++)
				{
					if (strcmp(shader.uavs[i].name, siDesc.Name) == 0)
					{
						merge = i;
						break;
					}
				...
				{
					size_t length = strlen(siDesc.Name);
					shader.uavs[shader.nUAVs].name = new char[length + 1];
					strcpy(shader.uavs[shader.nUAVs].name, siDesc.Name);
					shader.uavs[shader.nUAVs].vsIndex = -1;
					shader.uavs[shader.nUAVs].gsIndex = -1;
					shader.uavs[shader.nUAVs].psIndex = siDesc.BindPoint;
					shader.nUAVs++;
				}
				else
				{
					shader.uavs[merge].psIndex = siDesc.BindPoint;
				}
			}
			else if (siDesc.Type == D3D10_SIT_SAMPLER)
			{
...
VertexFormatID Direct3D11Renderer::addVertexFormat(const FormatDesc *formatDesc, const uint nAttribs, const ShaderID shader)
{
  ...
  for (uint i = 0; i < nAttribs; i++)
  {
    ...
    int component_size = formatDesc[i].size;
    ...
    desc[i].Format = formats[formatDesc[i].format][component_size - 1];
    desc[i].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
    desc[i].InstanceDataStepRate = 0;

    vf.vertexSize[stream] += component_size * getFormatSize(formatDesc[i].format);
  }
  ...
  if (FAILED(hr))
  {
    g_logger->debug(WIP_ERROR,"Couldn't create vertex declaration");
    return VF_NONE;
  }
...
void Direct3D11Renderer::removeVertexFormat(const VertexFormatID vf)
{
	vertexFormats[vf].inputLayout->Release();
	vertexFormats[vf].inputLayout = nullptr;
	memset(vertexFormats[vf].vertexSize, 0, 8 * sizeof(uint));
...
VertexBufferID Direct3D11Renderer::addVertexBuffer(const long size, const BufferAccess bufferAccess, const void *data)
{
  ...
  {
    g_logger->debug(WIP_ERROR,"Couldn't create vertex buffer");
    return VB_NONE;
  }
...
void Direct3D11Renderer::removeVertexBuffer(const VertexBufferID vb)
{
	vertexBuffers[vb].size = 0;
	vertexBuffers[vb].vertexBuffer->Release();
	vertexBuffers[vb].vertexBuffer = nullptr;
}
...
IndexBufferID Direct3D11Renderer::addIndexBuffer(const uint nIndices, const uint indexSize, const BufferAccess bufferAccess, const void *data)
{
  ...
  HRESULT hs;
  if (FAILED((hs = device->CreateBuffer(&desc, data ? &ibData : NULL, &ib.indexBuffer),hs)))
  {
    g_logger->debug(WIP_ERROR,"Couldn't create index buffer");
    return IB_NONE;
  }
...
void Direct3D11Renderer::removeIndexBuffer(const IndexBufferID ib)
{
	indexBuffers[ib].indexBuffer->Release();
	indexBuffers[ib].indexBuffer = nullptr;
	indexBuffers[ib].indexSize = 0;
	indexBuffers[ib].nIndices = 0;
}
...
SamplerStateID Direct3D11Renderer::addSamplerState(const Filter filter, const AddressMode s, const AddressMode t, const AddressMode r, const float lod, const uint maxAniso, const int compareFunc, const float *border_color)
{
  ...
  {
    g_logger->debug(WIP_ERROR,"Couldn't create samplerstate");
    return SS_NONE;
  }
...
BlendStateID Direct3D11Renderer::addBlendState(const int srcFactorRGB, const int destFactorRGB, const int srcFactorAlpha, const int destFactorAlpha, const int blendModeRGB, const int blendModeAlpha, const int mask, const bool alphaToCoverage)
{
  ...
  for (int i = 0; i < ARRAY_COUNT(desc.RenderTarget); i++)
  {
    ...
    rt.BlendOp = (D3D11_BLEND_OP)blendModeRGB;
  ...
  {
    g_logger->debug(WIP_ERROR,"Couldn't create blendstate");
    return BS_NONE;
  }
  ...
  const int depthFailFront, const int depthFailBack, const int stencilPassFront, const int stencilPassBack)
{
  ...
  {
    g_logger->debug(WIP_ERROR,"Couldn't create depthstate");
    return DS_NONE;
  }
...
RasterizerStateID Direct3D11Renderer::addRasterizerState(const int cullMode, const int fillMode, const bool multiSample, const bool scissor, const float depthBias, const float slopeDepthBias)
{
  ...
  {
    g_logger->debug(WIP_ERROR,"Couldn't create rasterizerstate");
    return RS_NONE;
  }
...
void Direct3D11Renderer::setStructreBufferAndApply(const char *bufferName, const StructureBufferID buffer)
{
	CHECK(selectedShader != SHADER_NONE);

	const Sampler *s = getSampler(shaders[selectedShader].textures, shaders[selectedShader].nTextures, bufferName);

	if (s)
	{
		if (s->gsIndex>=0)
		{
			context->GSSetShaderResources(s->gsIndex,1,&structureBuffers[buffer].srv);
		}
		if (s->vsIndex>=0)
		{
			context->VSSetShaderResources(s->vsIndex, 1, &structureBuffers[buffer].srv);
		}
		if (s->psIndex>=0)
		{
			context->PSSetShaderResources(s->psIndex, 1, &structureBuffers[buffer].srv);
		}
...
void Direct3D11Renderer::setUAV(const char *textureName, const TextureID texture)
{
  CHECK(selectedShader != SHADER_NONE);
  const Sampler *s = getSampler(shaders[selectedShader].textures, shaders[selectedShader].nTextures, textureName);
  if (s)
  {
    ...
    if (s->psIndex >= 0)
    {
      selectedTexturesUAVPS[s->psIndex] = texture;
...
bool fillUAV(ID3D11UnorderedAccessView **dest, int &min, int &max, 
  const TextureID selectedTextureUAV[],  TextureID currentTextureUAV[], const Texture *textures)
{
  min = 0;
  do
  {
    if (selectedTextureUAV[min] != currentTextureUAV[min])
    {
      max = MAX_TEXTUREUNIT;
      do
      {
        max--;
      } while (selectedTextureUAV[max] == currentTextureUAV[max]);

      for (int i = min; i <= max; i++)
      {
          if (selectedTextureUAV[i] != NO_UAV)
          {
            *dest++ = textures[selectedTextureUAV[i]].uav;
          }
          else
          {
            *dest++ = NULL;
          }
        currentTextureUAV[i] = selectedTextureUAV[i];
      }
      return true;
    }
    min++;
...
void Direct3D11Renderer::applyUAV(bool cu)
{

  if (cu)
  {
    CHECK(backBufferRTV);
    CHECK(depthBufferDSV);
    for (int i = 0; i < MAX_TEXTUREUNIT; ++i)
      currentTextureUAVPS[i] = NO_UAV;
    context->OMSetRenderTargetsAndUnorderedAccessViews(1, &backBufferRTV, depthBufferDSV, 0, 0, 0, 0);
    return;
  }

  
  ID3D11UnorderedAccessView *srViews[MAX_TEXTUREUNIT];

  int min, max;
  if (fillUAV(srViews, min, max, selectedTexturesUAVPS, currentTextureUAVPS, textures.data()))
    context->OMSetRenderTargetsAndUnorderedAccessViews(0,0,0,min, max - min + 1, srViews,0);
  printf("min:%d,max:%d\n",min,max);
...
void Direct3D11Renderer::changeRenderTargets(const TextureID *colorRTs, const uint nRenderTargets, const TextureID depthRT, const int depthSlice, const int *slices)
{
	...
	{
		if (!depthBufferDSV)
			g_logger->debug(WIP_WARNING, "depthBufferDSV is null!");
		context->OMSetRenderTargets(0, 0, depthBufferDSV);
	}
...
void Direct3D11Renderer::changeVertexBuffer(const int stream, const VertexBufferID vertexBufferID, const ptrdiff_t offset)
{
...
void Direct3D11Renderer::changeBlendState(const BlendStateID blendState, const uint sampleMask)
{
  if (blendState != currentBlendState || sampleMask != currentSampleMask)
  {
    ...
    if (blendState == BS_NONE)
      context->OMSetBlendState(NULL, a, sampleMask);
    else
      context->OMSetBlendState(blendStates[blendState].blendState, a, sampleMask);
...
void Direct3D11Renderer::clearRenderTarget(const TextureID renderTarget, const RBVector4 &color, const int slice)
{
  if (slice == NO_SLICE)
    context->ClearRenderTargetView(textures[renderTarget].rtv, (float*)&color);
  else
    context->ClearRenderTargetView(textures[renderTarget].rtvArray[slice], (float*)&color);
}
...
void Direct3D11Renderer::drawPlain(const Primitives primitives, RBVector2 *vertices, const uint nVertices, const BlendStateID blendState, const DepthStateID depthState, const RBVector4 *color)
{
  int size = nVertices * sizeof(RBVector2);

  UINT stride = sizeof(RBVector2);
  ...
  if (plainShader == SHADER_NONE)
  {
    ...
    plainVF = addVertexFormat(format, ARRAY_COUNT(format), plainShader);
  }

  RBVector4 col = RBVector4(1, 1, 1, 1);
  if (color) col = *color;
...
void Direct3D11Renderer::drawTextured(const Primitives primitives, TexVertex *vertices, const uint nVertices, const TextureID texture, const SamplerStateID samplerState, const BlendStateID blendState, const DepthStateID depthState, const RBVector4 *color)
{
  ...
  if (texShader == SHADER_NONE)
  {
    ...
    texVF = addVertexFormat(format, ARRAY_COUNT(format), texShader);
  }
...
ID3D11ShaderResourceView *Direct3D11Renderer::createSRV(ID3D11Resource *resource, DXGI_FORMAT format, const int firstSlice, const int sliceCount)
{
  ...
  case D3D11_RESOURCE_DIMENSION_TEXTURE3D:
    ...
    break;
  default:
    g_logger->debug(WIP_ERROR,"Unsupported type");
    return NULL;
  }

  if (FAILED(device->CreateShaderResourceView(resource, &srvDesc, &srv)))
  {
    g_logger->debug(WIP_ERROR,"CreateShaderResourceView failed");
    return NULL;
  }
...
ID3D11RenderTargetView *Direct3D11Renderer::createRTV(ID3D11Resource *resource, DXGI_FORMAT format, const int firstSlice, const int sliceCount)
{
  ...
  {
    g_logger->debug(WIP_ERROR,"CreateRenderTargetView failed");
    return NULL;
  }
...
ID3D11DepthStencilView *Direct3D11Renderer::createDSV(ID3D11Resource *resource, DXGI_FORMAT format, const int firstSlice, const int sliceCount)
{
  ...
  {
    g_logger->debug(WIP_ERROR,"CreateDepthStencilView failed");
    return NULL;
  }
...
u8 *Direct3D11Renderer::mapRollingVB(const uint size)
{
  CHECK(size <= ROLLING_VB_SIZE);
...
ID3D11ShaderResourceView* Direct3D11Renderer::getSRV(const TextureID texture) const
{
  return textures[texture].srv;
}

ID3D11RenderTargetView* Direct3D11Renderer::getRTV(const TextureID texture) const
{
  return textures[texture].rtv;
}

ID3D11DepthStencilView* Direct3D11Renderer::getDSV(const TextureID texture) const
{
