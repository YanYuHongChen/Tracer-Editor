...
public:
  ...
  //没有移动texture数组，因为这样会造成id失效，仅仅释放了资源，浪费了空间
  //todo:增加一个vector记录当前未被使用的id，有新申请时，顺序返回这些id，若此vector空则返回尾部id
  ...
  void removeIndexBuffer(const IndexBufferID ib);
